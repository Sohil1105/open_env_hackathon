...
# Update the executeStep function to include the stage-by-step toast/log animation
execute_step_animated = """// ─── Execute Pipeline ───────────────────────────────────────
async function executeStep() {
  const btn = document.getElementById('executeBtn');
  btn.disabled = true;
  btn.innerHTML = '<span class="spinner"></span>INITIALIZING AI AGENT...';
  ...
  try {
    ...
    // Determine risk stage for API compatibility
    ...
    // --- ANIMATED STAGES ---
    const stages = [
        "📡 Connecting to Global Credit Bureau...",
        "📂 STAGE 1: Documentation & Identity Check...",
        "🔍 STAGE 2: Credit Character Assessment...",
        "📊 STAGE 3: Capacity & DTI Analysis...",
        "⚖️ STAGE 4: Final Underwriting & Decision..."
    ];
    ...
    btn.innerHTML = '<span class="spinner"></span>FINALIZING VERDICT...';
    ...
    showToast('✨ Autonomous Underwriting Complete!', 'success');
...
if start_idx != -1 and end_idx != -1:
    content = content[:start_idx] + execute_step_animated + content[end_idx:]

with open("static/index.html", "w", encoding="utf-8") as f:
    f.write(content)
print("Patched static/index.html with interactive stages")
